...
* [Opt-in fractional client scaling](doc/fractional-scaling.md) with per-output preferences, independent of RandR transforms (requires client/toolkit support)
* Experimental [mixed native/legacy scaling](doc/mixed-scaling.md) on one output, with a compositor-facing physical-frame transport and maintained reference test compositor
